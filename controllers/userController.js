...
const userController = {
    register: async (req, res) => {
        var errores = []
        const {name, lastname, username, email, password, URLpic, country } = req.body
        ...
        if (errores.length === 0) {
            ...
            var newUser = new User({
                name, lastname, username, email, password: passwordHasheado, URLpic, country
            })
        ...
        return res.json({success: errores.length === 0 ? true : false, 
            errores: errores,
            response: errores.length === 0 && {token, name: newUserSaved.name, URLpic: newUserSaved.URLpic}})
    ...
    logFromLS: (req, res) => {
        res.json({success: true, response: {token: req.body.token, name: req.user.name, URLpic: req.user.URLpic}})
    }
}
